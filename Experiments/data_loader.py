...
CSV_PATH = "merged_frame.csv"
#CSV_PATH = "../complete-frame.csv"

CSV_MINER_PATH = "../testminereffectiveness-extended-with-mine.csv"
...
def load_frame(drop_project=True):
    ...
    frame2['project'] = frame2['Project']
    frame3['project'] = frame3['Project']


    frame3 = frame3.drop(['Bad API', 'Junit', 'Hamcrest', 'Mockito', 'Nº','Project'], axis=1)


    frame = pd.merge(frame1, frame2, on=['TestClassName','project'])
    frame = pd.merge(frame, frame3, on=['ClassName','project'])
    frame = frame.drop(['module', 'path_test','test_name','path_src',
                        'commit', 'class_name'], axis=1)
    
    if drop_project:
        frame = frame.drop(['project'], axis=1)
    frame = frame.sample(frac=1).reset_index(drop=True)
    frame = frame.dropna()
